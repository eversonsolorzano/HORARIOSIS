...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // Verificar si el usuario/email/documento ya existe
    ...
    $stmt = $db->prepare("SELECT id_estudiante FROM estudiantes WHERE documento_identidad = ?");
    ...
    if (empty($errores)) {
        try {
            ...
            $stmt->execute([$username, $password, $email, $programa_nombre]);
            ...
            $stmt = $db->prepare("INSERT INTO estudiantes 
                (id_usuario, id_programa, codigo_estudiante, nombres, apellidos, documento_identidad, 
                 fecha_nacimiento, genero, telefono, direccion, semestre_actual, fecha_ingreso) 
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, CURDATE())");
            
            $stmt->execute([
                $id_usuario, $id_programa, $codigo_estudiante, $nombres, $apellidos, $documento,
                $fecha_nacimiento, $genero, $telefono, $direccion, $semestre_actual
            ]);
            
            $db->commit();
            Funciones::redireccionar('index.php', 'Estudiante creado exitosamente');
            
        } catch (Exception $e) {
            $db->rollBack();
            $errores[] = 'Error al crear el estudiante: ' . $e->getMessage();
        }
    }
}
?>
...
<head>
    ...
    <title>Crear Estudiante - Sistema de Horarios</title>
    ...
    <div class="container">
        <div class="header">
            ...
            <div class="nav">
                ...
                <a href="crear.php" class="active"><i class="fas fa-user-plus"></i> Nuevo Estudiante</a>
            </div>
        </div>
